  ...
  const getProductImage = async (productId: string): Promise<string | null> => {
    try {
      ...
      if (!supabaseUrl || !supabaseKey || 
        ...
        // Silently return null when Supabase is not configured
        return null;
      }
      ...
      if (error) {
        ...
        console.warn(`⚠️ Erro de banco ao buscar imagem do produto ${cleanProductId}:`, error.message);
        return null;
      }
      ...
      const imageUrl = data.image.public_url;
      return imageUrl;
      ...
      if (err instanceof Error) {
        if (err.message === 'Timeout') {
          ...
          console.warn(`⚠️ Erro inesperado ao buscar imagem do produto ${productId}:`, err.message);
          return null;
        }
      } else {
        console.warn(`⚠️ Erro desconhecido ao buscar imagem do produto ${productId}:`, err);
        return null;
      }
    }
  };
